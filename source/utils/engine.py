...
from source.utils.metrics import eval_bleu, eval_distinct
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        self.patience = 0
        self.max_patience_num = max_patience_num
    ...
    def train_epoch(self):
        ...
        if self.generator is not None:
            ...
            message_prefix = "[Valid][{:2d}]".format(self.epoch)
            metrics_message, ppl = valid_mm.report_cum()
            self.logger.info("   ".join([message_prefix, metrics_message]))
            ...
            if self.lr_scheduler is not None:
                self.lr_scheduler.step(ppl)
            self.logger.info("\n")

            self.logger.info("Generation starts ...")
            gen_save_file = os.path.join(
                self.save_dir, "{}_valid_{}.result").format(self.data_prefix, self.epoch)
            gen_eval_metrics, gen_eval_message = evaluate_generation(generator=self.generator,
                                                                     data_iter=self.valid_iter,
                                                                     save_file=gen_save_file)
            cur_valid_metric = ppl - (gen_eval_metrics.bleu + gen_eval_metrics.dist) * 100 + 10
            ...
            if is_best:
                ...
                self.patience += 1
            self.save(is_best)
            self.logger.info(gen_eval_message)
    ...
    def train(self):
        ...
        for _ in range(self.epoch, self.num_epochs):
            if self.patience < self.max_patience_num:
                self.train_epoch()
            else:
                print('Early stopped!')
                break
    ...
    def save(self, is_best=False):
        ...
        if is_best:
            best_model_file = os.path.join(self.save_dir, f"best_{self.data_prefix}.model")
            best_train_file = os.path.join(self.save_dir, f"best_{self.data_prefix}.train")
...
def evaluate_generation(generator,
    ...
    refs = [[result.tgt] for result in results]
    ...
    bleu = eval_bleu(refs, hyps)
    report_message.append("Bleu4-{:.4f}".format(bleu))

    dist = eval_distinct(hyps)
    report_message.append("Distinct2-{:.4f}".format(dist))
    ...
    metrics = Pack()
    metrics.add(bleu=bleu, dist=dist)
