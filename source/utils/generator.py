...
class TopKGenerator(object):
    ...
    def __init__(self, model, config):
        self.model = model.cuda() if config.use_gpu else model
        self.src_field = config.src_field
        self.tgt_field = config.tgt_field
        # self.cue_field = config.cue_field
        self.k = 3
        self.max_length = config.max_dec_len
        self.ignore_unk = config.ignore_unk
        self.length_average = config.length_average
        self.use_gpu = config.use_gpu
        self.PAD = self.tgt_field.pad_token_id
        self.UNK = self.tgt_field.unk_token_id
        self.BOS = self.tgt_field.bos_token_id
        self.EOS = self.tgt_field.eos_token_id
    ...
    def generate(self, batch_iter, num_batches=None):
        ...
        for batch in batch_iter:
            ...
            src = batch.src[0]
            tgt = batch.tgt[0]
            src = self.src_field.decode(src)
            tgt = self.tgt_field.decode(tgt)
            preds = self.tgt_field.decode(preds)
